...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
        ...
        //BONUS2
        /*materialKd = glm::vec3(0.5, 0.7, 0.2);
        materialKs = glm::vec3(0.8, 0.4, 0.3);
        materialKa = glm::vec3(0.6, 0.1, 0.25);
        materialKe = glm::vec3(0.6, 0.3, 0.2);
        intensitate_ambientala_globala = glm::vec3(2, 0.5, 1.5);
        intensitate_luminoasa = glm::vec3(1.5, 1, 0.5);*/
    }
}
...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0.7, 0, 0));
    }

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(1, 0.6, 1));
    }
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.6, 1, 0.4));
    }
...
void Lab8::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, const glm::vec3& color)
{
    ...
    int spot_loc = glGetUniformLocation(shader->program, "is_spot");
    glUniform1i(spot_loc, spot);

    int angle_loc = glGetUniformLocation(shader->program, "cut_off");
    glUniform1f(angle_loc, RADIANS(angle));
...
{
    float speed = 2;
    float direction_speed = 0.5f;

    if ((!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT)) && (!window->KeyHold(GLFW_KEY_SPACE)))
    {
        ...
        if (window->KeyHold(GLFW_KEY_UP)) angle += deltaTime * 100;
        if (window->KeyHold(GLFW_KEY_DOWN)) angle -= deltaTime * 100;
    }

    if (window->KeyHold(GLFW_KEY_SPACE))
    {
        if (window->KeyHold(GLFW_KEY_W)) lightDirection.x += deltaTime * direction_speed;
        if (window->KeyHold(GLFW_KEY_A)) lightDirection.y += deltaTime * direction_speed;
        if (window->KeyHold(GLFW_KEY_S)) lightDirection.z += deltaTime * direction_speed;
        if (window->KeyHold(GLFW_KEY_D)) lightDirection.x -= deltaTime * direction_speed;
        if (window->KeyHold(GLFW_KEY_E)) lightDirection.y -= deltaTime * direction_speed;
        if (window->KeyHold(GLFW_KEY_Q)) lightDirection.z -= deltaTime * direction_speed;
    }
}
