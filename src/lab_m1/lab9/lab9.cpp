...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    int is_mixed = glGetUniformLocation(shader->program, "mixed");
    glUniform1i(is_mixed, mixed);

    int is_earth = glGetUniformLocation(shader->program, "isEarth");
    glUniform1i(is_earth, isEarth);

    int is_square = glGetUniformLocation(shader->program, "isSquare");
    glUniform1i(is_square, isSquare);

    int mouseMove = glGetUniformLocation(shader->program, "mouse");
    glUniform2f(mouseMove, mouse.x, mouse.y);
