...
void createCircle(int numberOfVetices, vector <VertexFormat>&vertices, vector <unsigned int>&indices);

void createCircle(int numberOfVetices, vector <VertexFormat>& vertices, vector <unsigned int>& indices)
{
    float radius = 1.0f;
    float Pi = M_PI;
    float alpha = (float)360 / (numberOfVertices - 2) * Pi / 180;

    vertices.push_back(VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0.676f, 0.977f, 0.133f), glm::vec3(0.2, 0.8, 0.6)));
    for (int i = 0; i <= numberOfVertices; i++)
        vertices.push_back(VertexFormat(glm::vec3(radius * cos((i - 1) * alpha), radius * sin((i - 1) * alpha), 0), glm::vec3(0.945f, 0.719f, 0.592f), glm::vec3(0.2, 0.8, 0.6)));
...
void Lab2::Init()
{
    ...
    // Create a mesh box using custom data
    {
        ...
        {
            VertexFormat(glm::vec3(0, 0,  1), glm::vec3(0.676f,  0.977f,  0.133f), glm::vec3(0.2, 0.8, 0.6)),
            // TODO(student): Complete the vertices data for the cube mesh
            VertexFormat(glm::vec3(1, 0, 1), glm::vec3(0.583f,  0.771f,  0.014f), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(0, 1, 1), glm::vec3(0.609f,  0.115f,  0.436f), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(1, 1, 1), glm::vec3(0.945f,  0.719f,  0.592f), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0.167f,  0.620f,  0.077f), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(1, 0, 0), glm::vec3(0.517f,  0.713f,  0.338f), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(0, 1, 0), glm::vec3(0.393f,  0.621f,  0.362f), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(1, 1, 0), glm::vec3(0.483f,  0.596f,  0.789f), glm::vec3(0.2, 0.8, 0.6)),
        };
    ...
    // by definition!
    {
        vector<VertexFormat> vertices
        {
            VertexFormat(glm::vec3(0, 0,  1), glm::vec3(0.676f,  0.977f,  0.133f), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(1, 0, 1), glm::vec3(0.583f,  0.771f,  0.014f), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(0.8, 1, 0.4), glm::vec3(0.609f,  0.115f,  0.436f), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(1, 0, 0), glm::vec3(0.945f,  0.719f,  0.592f), glm::vec3(0.2, 0.8, 0.6)),
        };
        vector<unsigned int> indices =
        {
            0, 1, 2,
    ...
    // opposing vertex orientations.
    {
        vector<VertexFormat> vertices
        {
            VertexFormat(glm::vec3(1, 2, 0), glm::vec3(0.676f,  0.977f,  0.133f), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(0, 2, 1), glm::vec3(0.583f,  0.771f,  0.014f), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(0, 1, 1), glm::vec3(0.609f,  0.115f,  0.436f), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(1, 1, 0), glm::vec3(0.945f,  0.719f,  0.592f), glm::vec3(0.2, 0.8, 0.6)),
        };
        vector<unsigned int> indices =
        {
            0, 3, 1,
...

    {
        ...
        createCircle(numberOfVertices, vertices, indices);

        Mesh* mesh = new Mesh("morisca");
        CreateMesh("morisca", vertices, indices);
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(-1.0f, 0, 1.5), glm::vec3(1.0f));

    // TODO(student): Draw the tetrahedron
    RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(-3.0f, 1.0f, 1), glm::vec3(1.0f));

    // TODO(student): Draw the square
    RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(1.0f, 0.0f, 1.0), glm::vec3(1.2f));

    RenderMesh(meshes["morisca"], shaders["VertexColor"], glm::vec3(4.0f, 1.0f, 2.0f), glm::vec3(1.0f));
...
void Lab2::OnKeyPress(int key, int mods)
{
    ...
    if ((key == GLFW_KEY_DOWN) && (numberOfVertices > 2))
    {
        numberOfVertices--;
        vector<VertexFormat> vertices;
        vector<unsigned int> indices;
